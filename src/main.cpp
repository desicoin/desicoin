...
static const int64 nTargetTimespan = 0.5 * 24 * 60 * 60; // Desicoin: 0.5 days
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "SA-AUS 2014 2nd T20I reduced to 7 overs each";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
